<template>
  <div class="container" :class="{ 'full-screen-container': isFullScreen }">
    <div class="demo">
      ...
      <iframe v-if="!typeMd" class="elp-iframe"
              :class="{ 'full-screen-iframe': isFullScreen ,'uni-app-box':props.libType==='uni-app'}"
              :src="`${baseUrl[props.libType]}#/${props.iframeSrc}`"
              :style="{ height: demoHeight }"
      />
      <div v-else class="elp-iframe">
        <slot />
      </div>
    </div>
    <div class="options">
...
const baseUrl = {
  ...
  'uni-app': import.meta.env.VITE_UNI_DEV_BASE
}

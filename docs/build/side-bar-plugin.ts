...
const componentBasePath = () => {
  return path.resolve(__dirname, '../zh-CN/front')
}
  ...
  return Promise.all(dirArr.map(async (dirItemPath: string) => {
    ...
    return {
      ...
      items: await Promise.all(fileArr.map(async (fileName: string) => {
        ...
        let sideName = reg.exec(fileStr) && (reg.exec(fileStr) as Array<string>)[0]
        return {
          text: sideName,
          link: `/zh-CN/front${path}/${dirItemPath}/${fileName.split('.')[0]}`
        }
      }))
    }
  }))
...
const mdFilePath = ['/element-plus', '/element-ui', '/ant-design','/uni-app']
